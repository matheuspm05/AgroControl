import { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import Button from "../../components/Button";
import FazendaGate from "../../components/FazendaGate";
import Input from "../../components/Input";
import SelectField from "../../components/SelectField";
import StatusBanner from "../../components/StatusBanner";
import { agroApi } from "../../services/api";
import { getApiErrorMessage } from "../../utils/apiError";
import { tipoVegetacaoOptions } from "../../utils/pastoModule";
import { formatArea, formatDate } from "../../utils/formatters";

function PastoEdit() {
  return (
    <FazendaGate>
      {(activeFazenda) => <PastoEditContent activeFazenda={activeFazenda} />}
    </FazendaGate>
  );
}

function PastoEditContent({ activeFazenda }) {
  const navigate = useNavigate();
  const { pastoId } = useParams();
  const [pasto, setPasto] = useState(null);
  const [animais, setAnimais] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [form, setForm] = useState({
    nome: "",
    tamanho: "",
    tipoVegetacao: "",
  });

  useEffect(() => {
    let isMounted = true;

    async function loadData() {
      try {
        setLoading(true);
        setError("");

        const [pastoData, animaisData] = await Promise.all([
          agroApi.getPasto(pastoId),
          agroApi.getPastoAnimais(pastoId),
        ]);

        if (!isMounted) {
          return;
        }

        setPasto(pastoData);
        setAnimais(animaisData);
        setForm({
          nome: pastoData.nome || "",
          tamanho:
            pastoData.tamanho === null || pastoData.tamanho === undefined
              ? ""
              : String(pastoData.tamanho),
          tipoVegetacao: pastoData.tipoVegetacao || "",
        });
      } catch (err) {
        if (isMounted) {
          setError(
            getApiErrorMessage(
              err,
              "Não foi possível carregar os dados do pasto.",
            ),
          );
        }
      } finally {
        if (isMounted) {
          setLoading(false);
        }
      }
    }

    loadData();

    return () => {
      isMounted = false;
    };
  }, [pastoId]);

  function updateField(field) {
    return (event) => {
      setForm((current) => ({
        ...current,
        [field]: event.target.value,
      }));
    };
  }

  async function handleSubmit(event) {
    event.preventDefault();

    if (!pasto) {
      return;
    }

    try {
      setSubmitting(true);
      setError("");

      await agroApi.updatePasto(pasto.id, {
        fazendaId: pasto.fazendaId,
        nome: form.nome,
        tamanho: form.tamanho === "" ? null : Number(form.tamanho),
        tipoVegetacao: form.tipoVegetacao || null,
        dataCadastro: pasto.dataCadastro,
      });

      navigate(`/pastos/${pasto.id}`);
    } catch (err) {
      setError(
        getApiErrorMessage(
          err,
          "Não foi possível salvar as alterações do pasto.",
        ),
      );
    } finally {
      setSubmitting(false);
    }
  }

  async function handleDelete() {
    if (!pasto) {
      return;
    }

    const confirmed = window.confirm(
      `Deseja excluir o pasto "${pasto.nome}"? Essa ação não pode ser desfeita.`,
    );

    if (!confirmed) {
      return;
    }

    try {
      setDeleting(true);
      setError("");
      await agroApi.deletePasto(pasto.id);
      navigate("/pastos");
    } catch (err) {
      setError(
        getApiErrorMessage(
          err,
          "Não foi possível excluir o pasto agora.",
        ),
      );
    } finally {
      setDeleting(false);
    }
  }

  const hasAnimaisVinculados = animais.length > 0;

  return (
    <div className="space-y-5">
      {error ? (
        <StatusBanner
          title="Não foi possível concluir esta operação"
          message={error}
        />
      ) : null}

      {pasto && pasto.fazendaId !== activeFazenda.id ? (
        <StatusBanner
          variant="info"
          title="Pasto carregado fora da fazenda ativa"
          message="A edição continua disponível, mas vale revisar a fazenda ativa do ambiente para manter o contexto alinhado."
        />
      ) : null}

      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-emerald-950/8 px-5 py-5 sm:px-6">
          <p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">
            Dados do pasto
          </p>
          <h2 className="mt-2 text-2xl font-bold text-emerald-950">
            {pasto ? `Editar ${pasto.nome}` : "Editar pasto"}
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Atualize os dados do pasto. A exclusão fica bloqueada enquanto houver
            animais vinculados a este local.
          </p>
        </div>

        {loading ? (
          <EditLoading />
        ) : pasto ? (
          <>
            <form onSubmit={handleSubmit} className="px-5 py-5 sm:px-6">
              <div className="grid gap-5 lg:grid-cols-2">
                <Input
                  label="Nome do pasto"
                  placeholder="Digite o nome do pasto"
                  value={form.nome}
                  onChange={updateField("nome")}
                  className="lg:col-span-2"
                  required
                />

                <Input
                  label="Tamanho"
                  type="number"
                  min="0"
                  step="0.01"
                  placeholder="Ex.: 20"
                  value={form.tamanho}
                  onChange={updateField("tamanho")}
                  rightElement={
                    <span className="text-sm font-medium text-slate-500">ha</span>
                  }
                />

                <SelectField
                  label="Tipo de vegetação"
                  value={form.tipoVegetacao}
                  onChange={updateField("tipoVegetacao")}
                  options={tipoVegetacaoOptions}
                  placeholder="Selecione uma vegetação"
                />
              </div>

              <div className="mt-8 grid gap-4 rounded-lg border border-slate-200 bg-slate-50 px-4 py-4 sm:grid-cols-2 xl:grid-cols-4">
                <DataItem label="Fazenda" value={`#${pasto.fazendaId}`} />
                <DataItem label="Tamanho atual" value={formatArea(pasto.tamanho)} />
                <DataItem
                  label="Cadastro"
                  value={formatDate(pasto.dataCadastro)}
                />
                <DataItem
                  label="Animais vinculados"
                  value={String(animais.length)}
                />
              </div>

              <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
                <Link
                  to={`/pastos/${pasto.id}`}
                  className="inline-flex items-center justify-center rounded-lg border border-emerald-950/15 bg-white px-5 py-3 text-base font-semibold text-emerald-950 transition hover:bg-emerald-50"
                >
                  Cancelar
                </Link>

                <Button type="submit" disabled={submitting}>
                  {submitting ? "Salvando..." : "Salvar alterações"}
                </Button>
              </div>
            </form>

            <div className="border-t border-slate-100 px-5 py-5 sm:px-6">
              {hasAnimaisVinculados ? (
                <StatusBanner
                  title="Não é possível excluir este pasto agora"
                  message="Existem animais vinculados a este pasto. Movimente os animais antes de tentar excluir."
                  variant="error"
                />
              ) : (
                <StatusBanner
                  title="Exclusão disponível"
                  message="Nenhum animal está vinculado a este pasto neste momento."
                  variant="warning"
                />
              )}

              <div className="mt-4 flex justify-end">
                <button
                  type="button"
                  onClick={handleDelete}
                  disabled={hasAnimaisVinculados || deleting}
                  className="inline-flex items-center justify-center rounded-lg border border-rose-200 bg-rose-50 px-5 py-3 text-base font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400"
                >
                  {deleting ? "Excluindo..." : "Excluir pasto"}
                </button>
              </div>
            </div>
          </>
        ) : (
          <div className="px-5 py-10 sm:px-6">
            <p className="text-base text-slate-600">
              O pasto solicitado não foi encontrado.
            </p>
          </div>
        )}
      </section>
    </div>
  );
}

function DataItem({ label, value }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
        {label}
      </p>
      <p className="mt-2 text-sm font-medium text-slate-700">{value}</p>
    </div>
  );
}

function EditLoading() {
  return (
    <div className="px-5 py-5 sm:px-6">
      <div className="grid gap-5 lg:grid-cols-2">
        {Array.from({ length: 5 }).map((_, index) => (
          <div
            key={index}
            className={`ag-skeleton animate-pulse rounded-lg ${
              index === 0 ? "lg:col-span-2" : ""
            } h-24`}
          />
        ))}
      </div>
    </div>
  );
}

export default PastoEdit;
