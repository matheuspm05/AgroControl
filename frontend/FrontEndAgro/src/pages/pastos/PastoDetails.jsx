...
import { formatArea, formatDate, formatWeight } from "../../utils/formatters";

function PastoDetails() {
  return (
    <FazendaGate>
      {(activeFazenda) => <PastoDetailsContent activeFazenda={activeFazenda} />}
    </FazendaGate>
  );
}

function PastoDetailsContent({ activeFazenda }) {
      ...
      try {
        ...
        if (isMounted) {
          setError(
            getApiErrorMessage(
              err,
              "Não foi possível carregar os detalhes do pasto.",
            ),
          );
        }
      } finally {
  ...
  return (
    <div className="space-y-5">
      {error ? (
        <StatusBanner
          title="Não foi possível carregar o pasto"
          message={error}
        />
      ) : null}
        ...
        <>
          <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-5 border-b border-emerald-950/8 px-5 py-5 sm:px-6 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <p className="mt-3 text-base text-slate-600">
                  ...
                  {pasto.tipoVegetacao || "Vegetação não informada"} •{" "}
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:w-auto">
                <ActionLink to="/pastos" variant="secondary">
                  Voltar para pastos
                </ActionLink>
                <ActionLink to={`/pastos/${pasto.id}/editar`} variant="secondary">
                  Editar
                </ActionLink>
              </div>
            </div>
            ...
            <div className="grid gap-4 px-5 py-5 sm:px-6 lg:grid-cols-2 xl:grid-cols-4">
              <SummaryCard label="Fazenda" value={`#${pasto.fazendaId}`} />
              <SummaryCard label="Tamanho" value={formatArea(pasto.tamanho)} />
              <SummaryCard
                label="Vegetação"
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                    Consulte os animais atualmente alocados neste pasto e siga
              ...
              <div className="px-5 py-12 text-center sm:px-6">
                <p className="text-lg font-semibold text-emerald-950">
                  {animais.length > 0
                    ...
                    : "Nenhum animal está neste pasto agora."}
                </p>
                <p className="mt-2 text-sm text-slate-600">
                  {animais.length > 0
                    ? "Ajuste os termos para encontrar um animal já vinculado ao pasto."
                    : "Quando houver animais com local atual neste pasto, eles aparecerão aqui."}
                </p>
              </div>
            ) : (
              <>
